...
export function verificar_paso_vocacional($id) {
    return http().get('/verificar-paso-vocacional?id='+$id);
}

export function guardar_respuesta_vocacional($data) {
    return http().post(
        '/guardar-respuesta-vocacional', 
        $data, 
        {
            headers: {
                'Content-Type': 'multipart/form-data'
